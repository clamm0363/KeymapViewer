...
- 視覚オプションの設定パネル表示中は callouts を一時的に非表示にし、レイアウト変化に伴う注釈ボックスの位置ずれを防ぎます。
- グローバルテーマ `DARK` 時のスロット外枠は背景とのコントラストを少し上げ、境界と面の差が読み取りやすいよう微調整します。
- `DARK` アプリ背景上で筐体テーマを `LIGHT` にした場合は、単調な明色面ではなくメタリックシルバー寄りのグラデーションと陰影を追加し、立体感を補強します。
- `CALLOUTS` はアクションボタンではなく状態トグルとして `ON/OFF` 表示へ変更し、選択中状態を `LAYER` 切替と同系統の青い強調で表現します。
- `EXPORT` は常時 primary 扱いをやめ、`SHARE` と同等の通常アクションボタンへ揃えます。
- `DARK` 筐体および dark tooltip 上のサブ情報文字は半段明るくして、補助情報の可読性を少し引き上げます。
