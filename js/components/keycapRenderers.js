...
export function renderBottomCaption({ label, isLight, color = null }) {
  ...
  const captionStyle = {
    ...
    ...(hasModifierSymbols
      ? {
          fontFamily:
            '"Segoe UI Symbol", "Noto Sans Symbols 2", "Apple Symbols", "Arial Unicode MS", sans-serif',
          letterSpacing: '0',
...
export function renderModifierSupplement({
  ...
  const shouldUseModifierSvg =
    keyStyle === 'Mac' && displayMode === 'Fluent' && !isNamedModifierComboLabel(label, modKeys);
