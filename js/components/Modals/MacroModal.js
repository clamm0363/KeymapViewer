...
export function MacroModal({ isLightApp, device, macroModalState, onClose, onUpdateDevice }) {
  ...
  const macrosToShow =
    specificMacroId !== null && specificMacroId !== undefined
      ...
      createElement(
        'div',
        {
          ...
          className:
            ...
            ' rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[80vh] flex flex-col',
            ...
            macrosToShow.length > 0
              ? macrosToShow.map((m) =>
                  createElement(
                    'div',
                    {
                      key: m.id,
                      className:
                        (isLightApp ? 'bg-slate-50' : 'bg-slate-950/50') +
                        ' grid grid-cols-[80px_1fr_2fr] gap-4 items-center p-3 rounded-xl border ' +
                        (isLightApp ? 'border-slate-200' : 'border-slate-800/50'),
                    },
                    [
                      createElement(
                        'div',
                        { key: 'id', className: 'text-xs font-black text-blue-500' },
                        'M(' + m.id + ')'
                      ),
                      createElement('input', {
                        key: 'input',
                        ...
                        className:
                          'bg-transparent border-b border-slate-700 outline-none text-sm font-bold w-full',
                        placeholder: 'Alias...',
                      }),
                      createElement(
                        'div',
                        ...
                        [
                          createElement(
                            'div',
                            {
                              key: 'content',
                              className:
                                'text-[11px] text-slate-500 break-all whitespace-pre-wrap leading-relaxed flex-1',
                                ...
                                'button',
                                {
                                  ...
                                  onClick: () => {
                                    navigator.clipboard
                                      ...
                                      .catch((err) =>
                                        console.error('Failed to copy macro content:', err)
                                      );
                                  },
                                  className:
                                    'p-1.5 rounded-lg border transition-all flex-shrink-0 flex items-center justify-center ' +
                                    (copiedId === m.id
                                      ? isLightApp
              ...
              : createElement(
                  'p',
                  {
                    className:
                      'text-center py-12 ' +
                      (isLightApp ? 'text-slate-500' : 'text-slate-400') +
                      ' text-xs font-bold uppercase tracking-widest leading-relaxed px-6',
                  },
