...
export function renderModKeycap({ model, isLight, isAppDark }) {
  const primaryModColor = getModColor(model.modKeys[0], isLight, isAppDark);
  const shouldCompactTapPrimary =
    !model.baseIsFluent && model.baseLabel && model.baseLabel.length >= 3;
      ...
      createElement(
        ...
        createElement(
          'div',
          {
            ...
            style: getMainLegendStyle(
              isLight,
              model.centerText,
              model.actuallyShowingSvg,
              model.kWidth,
              {
                color: primaryModColor,
                transform: 'none',
