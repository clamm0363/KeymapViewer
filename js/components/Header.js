...
export function Header({ isLightApp, layoutMode, deviceCount, onShowHelp, onShowLinks, onAddSlot, onSetLayoutMode, onSetAppTheme, appTheme }) {
    return createElement('header', { key: 'header', className: 'flex flex-col md:flex-row justify-between items-center mb-12 gap-6 w-full' }, [
        ...
        createElement('div', { key: 'actions', className: 'flex items-center gap-4 ' + (isLightApp ? 'bg-white/80 border-slate-200 shadow-sm' : 'bg-slate-900/50 border-slate-800') + ' p-2 rounded-2xl border backdrop-blur-sm transition-all' }, [
            ...
            createElement('button', { key: 'links-btn', onClick: onShowLinks, className: (isLightApp ? 'bg-slate-200 hover:bg-slate-300 text-slate-700' : 'bg-slate-700 hover:bg-slate-600 text-white') + ' px-4 py-2 rounded-xl text-[10px] font-black transition-all uppercase tracking-widest mr-1' }, 'LINKS'),
