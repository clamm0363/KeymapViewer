...
export function Header({ isLightApp, layoutMode, deviceCount, onShowHelp, onShowLinks, onMatchKeySize, onSetLayoutMode, onSetAppTheme, appTheme }) {
    return createElement('header', { key: 'header', className: 'sticky top-0 z-50 flex flex-col md:flex-row justify-between items-center gap-6 w-full py-4 md:py-6 mb-12 border-b backdrop-blur-md transition-all duration-300 ' + (isLightApp ? 'bg-slate-50/80 border-slate-200/50' : 'bg-slate-950/80 border-slate-900/50') }, [
        ...
        createElement('div', { key: 'actions', className: 'flex items-stretch gap-4 ' + (isLightApp ? 'bg-white/80 border-slate-200 shadow-sm' : 'bg-slate-900/50 border-slate-800') + ' p-2 rounded-2xl border backdrop-blur-sm transition-all' }, [
            ...
            createElement('div', { key: 'layout-toggle', className: 'flex self-stretch gap-1 border-r ' + (isLightApp ? 'border-slate-200' : 'border-slate-800') + ' pr-4 mr-2' }, [
                createElement('button', { key: 'grid-btn', onClick: () => onSetLayoutMode('grid'), className: (layoutMode === 'grid' ? 'bg-blue-600 text-white shadow-lg' : (isLightApp ? 'text-slate-400 hover:text-slate-600' : 'text-slate-500 hover:text-slate-300')) + ' flex items-center justify-center text-center px-4 py-2 rounded-xl text-xs font-bold transition-all' }, 'GRID'),
                createElement('button', { key: 'stack-btn', onClick: () => onSetLayoutMode('stack'), className: (layoutMode === 'stack' ? 'bg-blue-600 text-white shadow-lg' : (isLightApp ? 'text-slate-400 hover:text-slate-600' : 'text-slate-500 hover:text-slate-300')) + ' flex items-center justify-center text-center px-4 py-2 rounded-xl text-xs font-bold transition-all' }, 'STACK')
            ]),
            createElement('div', { key: 'theme-toggle', className: 'flex self-stretch items-stretch p-1 rounded-xl gap-1 ' + (isLightApp ? 'bg-slate-100' : 'bg-slate-800/50') }, [
                createElement('button', { key: 'light-btn', onClick: () => onSetAppTheme('light'), className: (isLightApp ? 'bg-white text-amber-500 shadow-sm' : 'text-slate-500 hover:text-slate-400') + ' w-9 h-full min-h-[45px] rounded-lg flex items-center justify-center transition-all', title: 'Light Mode', style: { fontFamily: 'Segoe Fluent Icons' } }, String.fromCharCode(0xE706)),
                createElement('button', { key: 'dark-btn', onClick: () => onSetAppTheme('dark'), className: (appTheme === 'dark' ? 'bg-slate-700 text-blue-400 shadow-sm' : 'text-slate-500 hover:text-slate-400') + ' w-9 h-full min-h-[45px] rounded-lg flex items-center justify-center transition-all', title: 'Dark Mode', style: { fontFamily: 'Segoe Fluent Icons' } }, String.fromCharCode(0xE708))
            ])
        ])
    ]);
}
