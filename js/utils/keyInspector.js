...
function normalizeInspectorLabel(label, fallbackLabel = '') {
    ...
    const fallback = String(fallbackLabel || '').replace(/\n/g, ' / ').trim();
    return fallback || normalized;
}

function buildResolvedLabel({ displayModel, parsed }) {
    if (displayModel?.route === 'layer') {
        return normalizeInspectorLabel(joinVisibleParts([
            displayModel.topTag,
    ...
    if (displayModel?.route === 'mod') {
        return normalizeInspectorLabel(joinVisibleParts([
            parsed.modLabel,
            parsed.baseLabel || displayModel.centerText
        ]), parsed.textFallback);
    }
    ...
    return normalizeInspectorLabel(displayModel?.centerText || parsed.displayText || '', parsed.textFallback);
}
...
function buildIconSummary({ displayModel, iconRenderState }) {
    ...
    return joinVisibleParts([
        iconKey,
        iconRenderState?.actuallyShowingSvg ? 'svg' : 'text'
    ]);
}
...
export function buildKeyInspectorData({
    ...
    return {
        inputCode: tooltipInfo.inputCode || String(code || '').trim().toUpperCase(),
        ...
        macro: macroInfo
            ? `M${macroInfo.macroId} = ${macroInfo.content || 'Empty'}`
            : ''
    };
}
