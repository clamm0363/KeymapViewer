...
async function sendViaCommand(device, commandId, payload = []) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            await device.sendReport(outputReportId, commandBuffer);
        } catch (error) {
            cleanup();
            reject(error);
        }
...
async function readKey(device, layer, row, col) {
    const response = await sendViaCommand(device, VIA_COMMANDS.dynamicKeymapGetKeycode, [layer, row, col]);
...
async function readKeymapBuffer(device, offset, size) {
    ...
    const response = await sendViaCommand(device, VIA_COMMANDS.dynamicKeymapGetBuffer, [offsetMsb, offsetLsb, size]);
...
async function readEncoderLayers(device, design, layerCount, protocolVersion, formatKeycode) {
    ...
    for (let layer = 0; layer < layerCount; layer += 1) {
        ...
        for (const encoderIndex of encoderIndices) {
            const ccwResponse = await sendViaCommand(device, VIA_COMMANDS.dynamicKeymapGetEncoder, [layer, encoderIndex, 0]);
            const cwResponse = await sendViaCommand(device, VIA_COMMANDS.dynamicKeymapGetEncoder, [layer, encoderIndex, 1]);
            encoderLayer.push([
...
async function readMacros(device, decoder = new TextDecoder()) {
    ...
    for (let offset = 0; offset < macroBufferSize; offset += VIA_DATA_BUFFER_SIZE) {
        ...
        const [offsetMsb, offsetLsb] = fromUint16(offset);
        const response = await sendViaCommand(device, VIA_COMMANDS.dynamicKeymapMacroGetBuffer, [offsetMsb, offsetLsb, VIA_DATA_BUFFER_SIZE]);
...
export async function readViaDeviceKeymap(device, definition) {
    ...
    try {
        ...
        let encoders = [];
        try {
            encoders = await readEncoderLayers(device, definition, layerCount, protocolVersion, formatViaKeycode);
        } catch (error) {
