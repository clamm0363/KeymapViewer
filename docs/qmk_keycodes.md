Title: Keycodes Overview | QMK Firmware

Source: https://docs.qmk.fm/keycodes

---

[Skip to content](https://docs.qmk.fm/keycodes#VPContent)
[QMK Firmware](https://docs.qmk.fm/)
[Home](https://docs.qmk.fm/)
Appearance

## Tutorial

[Introduction](https://docs.qmk.fm/newbs)
Introduction
[Setup](https://docs.qmk.fm/newbs_getting_started)
Setup
[Building Your First Firmware](https://docs.qmk.fm/newbs_building_firmware)
Building Your First Firmware
[Flashing Firmware](https://docs.qmk.fm/newbs_flashing)
Flashing Firmware
[Getting Help/Support](https://docs.qmk.fm/support)
Getting Help/Support
[External Userspace](https://docs.qmk.fm/newbs_external_userspace)
External Userspace
[Other Resources](https://docs.qmk.fm/newbs_learn_more_resources)
Other Resources
[Syllabus](https://docs.qmk.fm/syllabus)
Syllabus

## FAQs

[General FAQ](https://docs.qmk.fm/faq_general)
General FAQ
[Build/Compile QMK](https://docs.qmk.fm/faq_build)
Build/Compile QMK
[Troubleshooting QMK](https://docs.qmk.fm/faq_misc)
Troubleshooting QMK
[Debugging QMK](https://docs.qmk.fm/faq_debug)
Debugging QMK
[Keymap FAQ](https://docs.qmk.fm/faq_keymap)
Keymap FAQ
[Squeezing Space from AVR](https://docs.qmk.fm/squeezing_avr)
Squeezing Space from AVR
[Glossary](https://docs.qmk.fm/reference_glossary)
Glossary

## Licensing

[License Violations](https://docs.qmk.fm/license_violations)
License Violations
[Proprietary Libraries](https://docs.qmk.fm/proprietary_libs)
Proprietary Libraries

## Configurator

[Overview](https://docs.qmk.fm/newbs_building_firmware_configurator)
Overview
[Step by Step](https://docs.qmk.fm/configurator_step_by_step)
Step by Step
[Troubleshooting](https://docs.qmk.fm/configurator_troubleshooting)
Troubleshooting
[Architecture](https://docs.qmk.fm/configurator_architecture)
Architecture

[Overview](https://docs.qmk.fm/api_overview)
Overview
[API Documentation](https://docs.qmk.fm/api_docs)
API Documentation
[Keyboard Support](https://docs.qmk.fm/reference_configurator_support)
Keyboard Support
[Adding Default Keymaps](https://docs.qmk.fm/configurator_default_keymaps)
Adding Default Keymaps

[Overview](https://docs.qmk.fm/cli)
Overview
[Configuration](https://docs.qmk.fm/cli_configuration)
Configuration
[Commands](https://docs.qmk.fm/cli_commands)
Commands
[Tab Completion](https://docs.qmk.fm/cli_tab_complete)
Tab Completion

### Guides

[Customizing Functionality](https://docs.qmk.fm/custom_quantum_functions)
Customizing Functionality
[Driver Installation with Zadig](https://docs.qmk.fm/driver_installation_zadig)
Driver Installation with Zadig
[Community Modules](https://docs.qmk.fm/features/community_modules)
Community Modules
[Keymap Overview](https://docs.qmk.fm/keymap)
Keymap Overview

#### Development Environments

[Docker Guide](https://docs.qmk.fm/getting_started_docker)
Docker Guide
[Flashing](https://docs.qmk.fm/flashing)
Flashing

#### IDEs

[Using Eclipse with QMK](https://docs.qmk.fm/other_eclipse)
Using Eclipse with QMK
[Using VSCode with QMK](https://docs.qmk.fm/other_vscode)
Using VSCode with QMK

#### Git Best Practices

[Introduction](https://docs.qmk.fm/newbs_git_best_practices)
Introduction
[Your Fork](https://docs.qmk.fm/newbs_git_using_your_master_branch)
Your Fork
[Merge Conflicts](https://docs.qmk.fm/newbs_git_resolving_merge_conflicts)
Merge Conflicts
[Fixing Your Branch](https://docs.qmk.fm/newbs_git_resynchronize_a_branch)
Fixing Your Branch

### Simple Keycodes

[Full List](https://docs.qmk.fm/keycodes)
Full List
[Basic Keycodes](https://docs.qmk.fm/keycodes_basic)
Basic Keycodes
[Language-Specific Keycodes](https://docs.qmk.fm/reference_keymap_extras)
Language-Specific Keycodes
[Modifier Keys](https://docs.qmk.fm/feature_advanced_keycodes)
Modifier Keys
[Quantum Keycodes](https://docs.qmk.fm/quantum_keycodes)
Quantum Keycodes
[Magic Keycodes](https://docs.qmk.fm/keycodes_magic)
Magic Keycodes

### Advanced Keycodes

[Command](https://docs.qmk.fm/features/command)
Command
[Dynamic Macros](https://docs.qmk.fm/features/dynamic_macros)
Dynamic Macros
[Grave Escape](https://docs.qmk.fm/features/grave_esc)
Grave Escape
[Leader Key](https://docs.qmk.fm/features/leader_key)
Leader Key
[Mod-Tap](https://docs.qmk.fm/mod_tap)
Mod-Tap
[Macros](https://docs.qmk.fm/feature_macros)
Macros
[Mouse Keys](https://docs.qmk.fm/features/mouse_keys)
Mouse Keys
[Programmable Button](https://docs.qmk.fm/features/programmable_button)
Programmable Button
[Repeat Key](https://docs.qmk.fm/features/repeat_key)
Repeat Key
[Space Cadet Shift](https://docs.qmk.fm/features/space_cadet)
Space Cadet Shift
[US ANSI Shifted Keys](https://docs.qmk.fm/keycodes_us_ansi_shifted)
US ANSI Shifted Keys

### Software Features

[Auto Shift](https://docs.qmk.fm/features/auto_shift)
Auto Shift
[Autocorrect](https://docs.qmk.fm/features/autocorrect)
Autocorrect
[Caps Word](https://docs.qmk.fm/features/caps_word)
Caps Word
[Combos](https://docs.qmk.fm/features/combo)
Combos
[Debounce API](https://docs.qmk.fm/feature_debounce_type)
Debounce API
[Digitizer](https://docs.qmk.fm/features/digitizer)
Digitizer
[EEPROM](https://docs.qmk.fm/feature_eeprom)
EEPROM
[Key Lock](https://docs.qmk.fm/features/key_lock)
Key Lock
[Key Overrides](https://docs.qmk.fm/features/key_overrides)
Key Overrides
[Layers](https://docs.qmk.fm/feature_layers)
Layers
[Layer Lock](https://docs.qmk.fm/features/layer_lock)
Layer Lock
[One Shot Keys](https://docs.qmk.fm/one_shot_keys)
One Shot Keys
[OS Detection](https://docs.qmk.fm/features/os_detection)
OS Detection
[Raw HID](https://docs.qmk.fm/features/rawhid)
Raw HID
[Secure](https://docs.qmk.fm/features/secure)
Secure
[Send String](https://docs.qmk.fm/features/send_string)
Send String
[Sequencer](https://docs.qmk.fm/features/sequencer)
Sequencer
[Swap Hands](https://docs.qmk.fm/features/swap_hands)
Swap Hands
[Tap Dance](https://docs.qmk.fm/features/tap_dance)
Tap Dance
[Tap-Hold Configuration](https://docs.qmk.fm/tap_hold)
Tap-Hold Configuration
[Tri Layer](https://docs.qmk.fm/features/tri_layer)
Tri Layer
[Unicode](https://docs.qmk.fm/features/unicode)
Unicode
[Userspace](https://docs.qmk.fm/feature_userspace)
Userspace
[WPM Calculation](https://docs.qmk.fm/features/wpm)
WPM Calculation

### Hardware Features

#### Displays

[Quantum Painter](https://docs.qmk.fm/quantum_painter)

##### Quantum Painter

[Quantum Painter LVGL Integration](https://docs.qmk.fm/quantum_painter_lvgl)
Quantum Painter LVGL Integration
[HD44780 LCD Driver](https://docs.qmk.fm/features/hd44780)
HD44780 LCD Driver
[ST7565 LCD Driver](https://docs.qmk.fm/features/st7565)
ST7565 LCD Driver
[OLED Driver](https://docs.qmk.fm/features/oled_driver)
OLED Driver

#### Lighting

[Backlight](https://docs.qmk.fm/features/backlight)
Backlight
[LED Matrix](https://docs.qmk.fm/features/led_matrix)
LED Matrix
[RGB Lighting](https://docs.qmk.fm/features/rgblight)
RGB Lighting
[RGB Matrix](https://docs.qmk.fm/features/rgb_matrix)
RGB Matrix
[Audio](https://docs.qmk.fm/features/audio)
Audio
[Battery](https://docs.qmk.fm/features/battery)
Battery
[Bootmagic](https://docs.qmk.fm/features/bootmagic)
Bootmagic
[Converters](https://docs.qmk.fm/feature_converters)
Converters
[Custom Matrix](https://docs.qmk.fm/custom_matrix)
Custom Matrix
[DIP Switch](https://docs.qmk.fm/features/dip_switch)
DIP Switch
[Encoders](https://docs.qmk.fm/features/encoders)
Encoders
[Haptic Feedback](https://docs.qmk.fm/features/haptic_feedback)
Haptic Feedback
[Joystick](https://docs.qmk.fm/features/joystick)
Joystick
[LED Indicators](https://docs.qmk.fm/features/led_indicators)
LED Indicators
[MIDI](https://docs.qmk.fm/features/midi)
MIDI
[Pointing Device](https://docs.qmk.fm/features/pointing_device)
Pointing Device
[PS/2 Mouse](https://docs.qmk.fm/features/ps2_mouse)
PS/2 Mouse
[Split Keyboard](https://docs.qmk.fm/features/split_keyboard)
Split Keyboard
[Stenography](https://docs.qmk.fm/features/stenography)
Stenography
[Wireless](https://docs.qmk.fm/features/wireless)
Wireless

### Keyboard Building

[Easy Maker for One Offs](https://docs.qmk.fm/easy_maker)
Easy Maker for One Offs
[Porting Keyboards](https://docs.qmk.fm/porting_your_keyboard_to_qmk)
Porting Keyboards
[Hand Wiring Guide](https://docs.qmk.fm/hand_wire)
Hand Wiring Guide
[ISP Flashing Guide](https://docs.qmk.fm/isp_flashing_guide)
ISP Flashing Guide

[PR Checklist](https://docs.qmk.fm/pr_checklist)
PR Checklist

### Breaking Changes

[Overview](https://docs.qmk.fm/breaking_changes)
Overview
[My Pull Request Was Flagged](https://docs.qmk.fm/breaking_changes_instructions)
My Pull Request Was Flagged
[Most Recent ChangeLog](https://docs.qmk.fm/ChangeLog/20260222)
Most Recent ChangeLog
[Past Breaking Changes](https://docs.qmk.fm/breaking_changes_history)
Past Breaking Changes
[Deprecation Policy](https://docs.qmk.fm/support_deprecation_policy)
Deprecation Policy

### C Development

[ARM Debugging Guide](https://docs.qmk.fm/arm_debugging)
ARM Debugging Guide
[Coding Conventions](https://docs.qmk.fm/coding_conventions_c)
Coding Conventions
[Compatible Microcontrollers](https://docs.qmk.fm/compatible_microcontrollers)
Compatible Microcontrollers
[Drivers](https://docs.qmk.fm/hardware_drivers)

#### Drivers

[ADC Driver](https://docs.qmk.fm/drivers/adc)
ADC Driver
[APA102 Driver](https://docs.qmk.fm/drivers/apa102)
APA102 Driver
[Audio Driver](https://docs.qmk.fm/drivers/audio)
Audio Driver
[Battery Driver](https://docs.qmk.fm/drivers/battery)
Battery Driver
[EEPROM Driver](https://docs.qmk.fm/drivers/eeprom)
EEPROM Driver
[Flash Driver](https://docs.qmk.fm/drivers/flash)
Flash Driver
[I2C Driver](https://docs.qmk.fm/drivers/i2c)
I2C Driver
['serial' Driver](https://docs.qmk.fm/drivers/serial)
'serial' Driver
[SPI Driver](https://docs.qmk.fm/drivers/spi)
SPI Driver
[UART Driver](https://docs.qmk.fm/drivers/uart)
UART Driver
[WS2812 Driver](https://docs.qmk.fm/drivers/ws2812)
WS2812 Driver
[GPIO Controls](https://docs.qmk.fm/drivers/gpio)
GPIO Controls
[Keyboard Guidelines](https://docs.qmk.fm/hardware_keyboard_guidelines)
Keyboard Guidelines

### Python Development

[Coding Conventions](https://docs.qmk.fm/coding_conventions_python)
Coding Conventions
[QMK CLI Development](https://docs.qmk.fm/cli_development)
QMK CLI Development

### Configurator Development

#### QMK API

[Development Environment](https://docs.qmk.fm/api_development_environment)
Development Environment
[Architecture Overview](https://docs.qmk.fm/api_development_overview)
Architecture Overview

### Hardware Platform Development

#### Arm/ChibiOS

[Selecting an MCU](https://docs.qmk.fm/platformdev_selecting_arm_mcu)
Selecting an MCU
[Early initialization](https://docs.qmk.fm/platformdev_chibios_earlyinit)
Early initialization
[Raspberry Pi RP2040](https://docs.qmk.fm/platformdev_rp2040)
Raspberry Pi RP2040
[Proton C](https://docs.qmk.fm/platformdev_proton_c)
Proton C
[WeAct Blackpill F4x1](https://docs.qmk.fm/platformdev_blackpill_f4x1)
WeAct Blackpill F4x1

### QMK Reference

[Contributing to QMK](https://docs.qmk.fm/contributing)
Contributing to QMK
[Config Options](https://docs.qmk.fm/config_options)
Config Options
[Data Driven Configuration](https://docs.qmk.fm/data_driven_config)
Data Driven Configuration
[Make Documentation](https://docs.qmk.fm/getting_started_make_guide)
Make Documentation
[Documentation Best Practices](https://docs.qmk.fm/documentation_best_practices)
Documentation Best Practices
[Documentation Templates](https://docs.qmk.fm/documentation_templates)
Documentation Templates
[Community Layouts](https://docs.qmk.fm/feature_layouts)
Community Layouts
[Unit Testing](https://docs.qmk.fm/unit_testing)
Unit Testing
[Useful Functions](https://docs.qmk.fm/ref_functions)
Useful Functions
[info.json Format](https://docs.qmk.fm/reference_info_json)
info.json Format

### For a Deeper Understanding

[How Keyboards Work](https://docs.qmk.fm/how_keyboards_work)
How Keyboards Work
[How a Matrix Works](https://docs.qmk.fm/how_a_matrix_works)
How a Matrix Works
[Understanding QMK](https://docs.qmk.fm/understanding_qmk)
Understanding QMK

[​](https://docs.qmk.fm/keycodes#keycodes-overview)
When defining a [keymap](https://docs.qmk.fm/keymap) each key needs a valid key definition. This page documents the symbols that correspond to keycodes that are available to you in QMK.
This is a reference only. Each group of keys links to the page documenting their functionality in more detail.

[​](https://docs.qmk.fm/keycodes#basic-keycodes)
See also: [Basic Keycodes](https://docs.qmk.fm/keycodes_basic)

```
KC_NO
```

```
XXXXXXX
```

```
KC_TRANSPARENT
```

```
KC_TRNS
```

```
_______
```

```
KC_A
```

```
a
```

```
A
```

```
KC_B
```

```
b
```

```
B
```

```
KC_C
```

```
c
```

```
C
```

```
KC_D
```

```
d
```

```
D
```

```
KC_E
```

```
e
```

```
E
```

```
KC_F
```

```
f
```

```
F
```

```
KC_G
```

```
g
```

```
G
```

```
KC_H
```

```
h
```

```
H
```

```
KC_I
```

```
i
```

```
I
```

```
KC_J
```

```
j
```

```
J
```

```
KC_K
```

```
k
```

```
K
```

```
KC_L
```

```
l
```

```
L
```

```
KC_M
```

```
m
```

```
M
```

```
KC_N
```

```
n
```

```
N
```

```
KC_O
```

```
o
```

```
O
```

```
KC_P
```

```
p
```

```
P
```

```
KC_Q
```

```
q
```

```
Q
```

```
KC_R
```

```
r
```

```
R
```

```
KC_S
```

```
s
```

```
S
```

```
KC_T
```

```
t
```

```
T
```

```
KC_U
```

```
u
```

```
U
```

```
KC_V
```

```
v
```

```
V
```

```
KC_W
```

```
w
```

```
W
```

```
KC_X
```

```
x
```

```
X
```

```
KC_Y
```

```
y
```

```
Y
```

```
KC_Z
```

```
z
```

```
Z
```

```
KC_1
```

```
1
```

```
!
```

```
KC_2
```

```
2
```

```
@
```

```
KC_3
```

```
3
```

```
#
```

```
KC_4
```

```
4
```

```
$
```

```
KC_5
```

```
5
```

```
%
```

```
KC_6
```

```
6
```

```
^
```

```
KC_7
```

```
7
```

```
&
```

```
KC_8
```

```
8
```

```
*
```

```
KC_9
```

```
9
```

```
(
```

```
KC_0
```

```
0
```

```
)
```

```
KC_ENTER
```

```
KC_ENT
```

```
KC_ESCAPE
```

```
KC_ESC
```

```
KC_BACKSPACE
```

```
KC_BSPC
```

```
KC_TAB
```

```
KC_SPACE
```

```
KC_SPC
```

```
KC_MINUS
```

```
KC_MINS
```

```
-
```

```
_
```

```
KC_EQUAL
```

```
KC_EQL
```

```
=
```

```
+
```

```
KC_LEFT_BRACKET
```

```
KC_LBRC
```

```
[
```

```
{
```

```
KC_RIGHT_BRACKET
```

```
KC_RBRC
```

```
]
```

```
}
```

```
KC_BACKSLASH
```

```
KC_BSLS
```

```
\
```

```
|
```

```
KC_NONUS_HASH
```

```
KC_NUHS
```

```
#
```

```
~
```

```
KC_SEMICOLON
```

```
KC_SCLN
```

```
;
```

```
:
```

```
KC_QUOTE
```

```
KC_QUOT
```

```
'
```

```
"
```

```
KC_GRAVE
```

```
KC_GRV
```

```
`
```

```
~
```

```
KC_COMMA
```

```
KC_COMM
```

```
,
```

```
<
```

```
KC_DOT
```

```
.
```

```
>
```

```
KC_SLASH
```

```
KC_SLSH
```

```
/
```

```
?
```

```
KC_CAPS_LOCK
```

```
KC_CAPS
```

```
KC_F1
```

```
KC_F2
```

```
KC_F3
```

```
KC_F4
```

```
KC_F5
```

```
KC_F6
```

```
KC_F7
```

```
KC_F8
```

```
KC_F9
```

```
KC_F10
```

```
KC_F11
```

```
KC_F12
```

```
KC_PRINT_SCREEN
```

```
KC_PSCR
```

```
KC_SCROLL_LOCK
```

```
KC_SCRL
```

```
KC_BRMD
```

```
KC_PAUSE
```

```
KC_PAUS
```

```
KC_BRK
```

```
KC_BRMU
```

```
KC_INSERT
```

```
KC_INS
```

```
KC_HOME
```

```
KC_PAGE_UP
```

```
KC_PGUP
```

```
KC_DELETE
```

```
KC_DEL
```

```
KC_END
```

```
KC_PAGE_DOWN
```

```
KC_PGDN
```

```
KC_RIGHT
```

```
KC_RGHT
```

```
KC_LEFT
```

```
KC_DOWN
```

```
KC_UP
```

```
KC_NUM_LOCK
```

```
KC_NUM
```

```
KC_KP_SLASH
```

```
KC_PSLS
```

```
/
```

```
KC_KP_ASTERISK
```

```
KC_PAST
```

```
*
```

```
KC_KP_MINUS
```

```
KC_PMNS
```

```
-
```

```
KC_KP_PLUS
```

```
KC_PPLS
```

```
+
```

```
KC_KP_ENTER
```

```
KC_PENT
```

```
KC_KP_1
```

```
KC_P1
```

```
1
```

```
KC_KP_2
```

```
KC_P2
```

```
2
```

```
KC_KP_3
```

```
KC_P3
```

```
3
```

```
KC_KP_4
```

```
KC_P4
```

```
4
```

```
KC_KP_5
```

```
KC_P5
```

```
5
```

```
KC_KP_6
```

```
KC_P6
```

```
6
```

```
KC_KP_7
```

```
KC_P7
```

```
7
```

```
KC_KP_8
```

```
KC_P8
```

```
8
```

```
KC_KP_9
```

```
KC_P9
```

```
9
```

```
KC_KP_0
```

```
KC_P0
```

```
0
```

```
KC_KP_DOT
```

```
KC_PDOT
```

```
.
```

```
KC_NONUS_BACKSLASH
```

```
KC_NUBS
```

```
\
```

```
|
```

```
KC_APPLICATION
```

```
KC_APP
```

```
KC_KB_POWER
```

```
KC_KP_EQUAL
```

```
KC_PEQL
```

```
=
```

```
KC_F13
```

```
KC_F14
```

```
KC_F15
```

```
KC_F16
```

```
KC_F17
```

```
KC_F18
```

```
KC_F19
```

```
KC_F20
```

```
KC_F21
```

```
KC_F22
```

```
KC_F23
```

```
KC_F24
```

```
KC_EXECUTE
```

```
KC_EXEC
```

```
KC_HELP
```

```
KC_MENU
```

```
KC_SELECT
```

## Basic Keycodes [​](https://docs.qmk.fm/keycodes#basic-keycodes)

```
KC_SLCT
```

```
KC_STOP
```

```
KC_AGAIN
```

```
KC_AGIN
```

```
KC_UNDO
```

```
KC_CUT
```

```
KC_COPY
```

```
KC_PASTE
```

```
KC_PSTE
```

```
KC_FIND
```

```
KC_KB_MUTE
```

```
KC_KB_VOLUME_UP
```

```
KC_KB_VOLUME_DOWN
```

```
KC_LOCKING_CAPS_LOCK
```

```
KC_LCAP
```

```
KC_LOCKING_NUM_LOCK
```

```
KC_LNUM
```

```
KC_LOCKING_SCROLL_LOCK
```

```
KC_LSCR
```

```
KC_KP_COMMA
```

```
KC_PCMM
```

```
,
```

```
KC_KP_EQUAL_AS400
```

```
=
```

```
KC_INTERNATIONAL_1
```

```
KC_INT1
```

```
KC_INTERNATIONAL_2
```

```
KC_INT2
```

```
KC_INTERNATIONAL_3
```

```
KC_INT3
```

```
KC_INTERNATIONAL_4
```

```
KC_INT4
```

```
KC_INTERNATIONAL_5
```

```
KC_INT5
```

```
KC_INTERNATIONAL_6
```

```
KC_INT6
```

```
KC_INTERNATIONAL_7
```

```
KC_INT7
```

```
KC_INTERNATIONAL_8
```

```
KC_INT8
```

```
KC_INTERNATIONAL_9
```

```
KC_INT9
```

```
KC_LANGUAGE_1
```

```
KC_LNG1
```

```
KC_LANGUAGE_2
```

```
KC_LNG2
```

```
KC_LANGUAGE_3
```

```
KC_LNG3
```

```
KC_LANGUAGE_4
```

```
KC_LNG4
```

```
KC_LANGUAGE_5
```

```
KC_LNG5
```

```
KC_LANGUAGE_6
```

```
KC_LNG6
```

```
KC_LANGUAGE_7
```

```
KC_LNG7
```

```
KC_LANGUAGE_8
```

```
KC_LNG8
```

```
KC_LANGUAGE_9
```

```
KC_LNG9
```

```
KC_ALTERNATE_ERASE
```

```
KC_ERAS
```

```
KC_SYSTEM_REQUEST
```

```
KC_SYRQ
```

```
KC_CANCEL
```

```
KC_CNCL
```

```
KC_CLEAR
```

```
KC_CLR
```

```
KC_PRIOR
```

```
KC_PRIR
```

```
KC_RETURN
```

```
KC_RETN
```

```
KC_SEPARATOR
```

```
KC_SEPR
```

```
KC_OUT
```

```
KC_OPER
```

```
KC_CLEAR_AGAIN
```

```
KC_CLAG
```

```
KC_CRSEL
```

```
KC_CRSL
```

```
KC_EXSEL
```

```
KC_EXSL
```

```
KC_LEFT_CTRL
```

```
KC_LCTL
```

```
KC_LEFT_SHIFT
```

```
KC_LSFT
```

```
KC_LEFT_ALT
```

```
KC_LALT
```

```
KC_LOPT
```

```
KC_LEFT_GUI
```

```
KC_LGUI
```

```
KC_LCMD
```

```
KC_LWIN
```

```
KC_RIGHT_CTRL
```

```
KC_RCTL
```

```
KC_RIGHT_SHIFT
```

```
KC_RSFT
```

```
KC_RIGHT_ALT
```

```
KC_RALT
```

```
KC_ROPT
```

```
KC_ALGR
```

```
KC_RIGHT_GUI
```

```
KC_RGUI
```

```
KC_RCMD
```

```
KC_RWIN
```

```
KC_SYSTEM_POWER
```

```
KC_PWR
```

```
KC_SYSTEM_SLEEP
```

```
KC_SLEP
```

```
KC_SYSTEM_WAKE
```

```
KC_WAKE
```

```
KC_AUDIO_MUTE
```

```
KC_MUTE
```

```
KC_AUDIO_VOL_UP
```

```
KC_VOLU
```

```
KC_AUDIO_VOL_DOWN
```

```
KC_VOLD
```

```
KC_MEDIA_NEXT_TRACK
```

```
KC_MNXT
```

```
KC_MEDIA_PREV_TRACK
```

```
KC_MPRV
```

```
KC_MEDIA_STOP
```

```
KC_MSTP
```

```
KC_MEDIA_PLAY_PAUSE
```

```
KC_MPLY
```

```
KC_MEDIA_SELECT
```

```
KC_MSEL
```

```
KC_MEDIA_EJECT
```

```
KC_EJCT
```

```
KC_MAIL
```

```
KC_CALCULATOR
```

```
KC_CALC
```

```
KC_MY_COMPUTER
```

```
KC_MYCM
```

```
KC_WWW_SEARCH
```

```
KC_WSCH
```

```
KC_WWW_HOME
```

```
KC_WHOM
```

```
KC_WWW_BACK
```

```
KC_WBAK
```

```
KC_WWW_FORWARD
```

```
KC_WFWD
```

```
KC_WWW_STOP
```

```
KC_WSTP
```

```
KC_WWW_REFRESH
```

```
KC_WREF
```

```
KC_WWW_FAVORITES
```

```
KC_WFAV
```

```
KC_MEDIA_FAST_FORWARD
```

```
KC_MFFD
```

```
KC_MEDIA_REWIND
```

```
KC_MRWD
```

```
KC_BRIGHTNESS_UP
```

```
KC_BRIU
```

```
KC_BRIGHTNESS_DOWN
```

```
KC_BRID
```

```
KC_CONTROL_PANEL
```

```
KC_CPNL
```

```
KC_ASSISTANT
```

```
KC_ASST
```

```
KC_MISSION_CONTROL
```

```
KC_MCTL
```

```
KC_LAUNCHPAD
```

```
KC_LPAD
```

1. The Linux kernel HID driver recognizes [nearly all keycodes](https://github.com/torvalds/linux/blob/master/drivers/hid/hid-input.c), but the default bindings depend on the DE/WM.2. Treated as F13-F15.3. Must be held for about three seconds, and will display a prompt instead.4. Holding Shift+Option allows for finer control of volume level.5. Skips the entire track in iTunes when tapped, seeks within the current track when held.6. WMP does not recognize the Rewind key, but both alter playback speed in VLC.

## Quantum Keycodes [​](https://docs.qmk.fm/keycodes#quantum-keycodes)

[​](https://docs.qmk.fm/keycodes#quantum-keycodes)
See also: [Quantum Keycodes](https://docs.qmk.fm/quantum_keycodes#qmk-keycodes)

```
QK_BOOTLOADER
```

```
QK_BOOT
```

```
QK_DEBUG_TOGGLE
```

```
DB_TOGG
```

```
QK_CLEAR_EEPROM
```

```
EE_CLR
```

```
QK_MAKE
```

```
qmk compile -kb (keyboard) -km (keymap)
```

```
qmk flash
```

```
QK_REBOOT
```

```
QK_RBT
```

## Audio Keys [​](https://docs.qmk.fm/keycodes#audio-keys)

[​](https://docs.qmk.fm/keycodes#audio-keys)
See also: [Audio](https://docs.qmk.fm/features/audio)

```
QK_AUDIO_ON
```

```
AU_ON
```

```
QK_AUDIO_OFF
```

```
AU_OFF
```

```
QK_AUDIO_TOGGLE
```

```
AU_TOGG
```

```
QK_AUDIO_CLICKY_TOGGLE
```

```
CK_TOGG
```

```
QK_AUDIO_CLICKY_ON
```

```
CK_ON
```

```
QK_AUDIO_CLICKY_OFF
```

```
CK_OFF
```

```
QK_AUDIO_CLICKY_UP
```

```
CK_UP
```

```
QK_AUDIO_CLICKY_DOWN
```

```
CK_DOWN
```

```
QK_AUDIO_CLICKY_RESET
```

```
CK_RST
```

```
QK_MUSIC_ON
```

```
MU_ON
```

```
QK_MUSIC_OFF
```

```
MU_OFF
```

```
QK_MUSIC_TOGGLE
```

```
MU_TOGG
```

```
QK_MUSIC_MODE_NEXT
```

```
MU_NEXT
```

```
QK_AUDIO_VOICE_NEXT
```

```
AU_NEXT
```

```
QK_AUDIO_VOICE_PREVIOUS
```

```
AU_PREV
```

## Auto Shift [​](https://docs.qmk.fm/keycodes#auto-shift)

[​](https://docs.qmk.fm/keycodes#auto-shift)
See also: [Auto Shift](https://docs.qmk.fm/features/auto_shift)

```
QK_AUTO_SHIFT_DOWN
```

```
AS_DOWN
```

```
QK_AUTO_SHIFT_UP
```

```
AS_UP
```

```
QK_AUTO_SHIFT_REPORT
```

```
AS_RPT
```

```
QK_AUTO_SHIFT_ON
```

```
AS_ON
```

```
QK_AUTO_SHIFT_OFF
```

```
AS_OFF
```

```
QK_AUTO_SHIFT_TOGGLE
```

```
AS_TOGG
```

## Autocorrect [​](https://docs.qmk.fm/keycodes#autocorrect)

[​](https://docs.qmk.fm/keycodes#autocorrect)
See also: [Autocorrect](https://docs.qmk.fm/features/autocorrect)

```
QK_AUTOCORRECT_ON
```

```
AC_ON
```

```
QK_AUTOCORRECT_OFF
```

```
AC_OFF
```

```
QK_AUTOCORRECT_TOGGLE
```

```
AC_TOGG
```

## Backlighting [​](https://docs.qmk.fm/keycodes#backlighting)

[​](https://docs.qmk.fm/keycodes#backlighting)
See also: [Backlighting](https://docs.qmk.fm/features/backlight)

```
QK_BACKLIGHT_TOGGLE
```

```
BL_TOGG
```

```
QK_BACKLIGHT_STEP
```

```
BL_STEP
```

```
QK_BACKLIGHT_ON
```

```
BL_ON
```

```
QK_BACKLIGHT_OFF
```

```
BL_OFF
```

```
QK_BACKLIGHT_UP
```

```
BL_UP
```

```
QK_BACKLIGHT_DOWN
```

```
BL_DOWN
```

```
QK_BACKLIGHT_TOGGLE_BREATHING
```

```
BL_BRTG
```

## Wireless/Bluetooth [​](https://docs.qmk.fm/keycodes#bluetooth)

[​](https://docs.qmk.fm/keycodes#bluetooth)
See also: [Wireless](https://docs.qmk.fm/features/wireless)

```
QK_OUTPUT_AUTO
```

```
OU_AUTO
```

```
QK_OUTPUT_NEXT
```

```
OU_NEXT
```

```
QK_OUTPUT_PREV
```

```
OU_PREV
```

```
QK_OUTPUT_NONE
```

```
OU_NONE
```

```
QK_OUTPUT_USB
```

```
OU_USB
```

```
QK_OUTPUT_2P4GHZ
```

```
OU_2P4G
```

```
QK_OUTPUT_BLUETOOTH
```

```
OU_BT
```

```
QK_BLUETOOTH_PROFILE_NEXT
```

```
BT_NEXT
```

```
QK_BLUETOOTH_PROFILE_PREV
```

```
BT_PREV
```

```
QK_BLUETOOTH_UNPAIR
```

```
BT_UNPR
```

```
QK_BLUETOOTH_PROFILE1
```

```
BT_PRF1
```

```
QK_BLUETOOTH_PROFILE2
```

```
BT_PRF2
```

```
QK_BLUETOOTH_PROFILE3
```

```
BT_PRF3
```

```
QK_BLUETOOTH_PROFILE4
```

```
BT_PRF4
```

```
QK_BLUETOOTH_PROFILE5
```

```
BT_PRF5
```

## Caps Word [​](https://docs.qmk.fm/keycodes#caps-word)

[​](https://docs.qmk.fm/keycodes#caps-word)
See also: [Caps Word](https://docs.qmk.fm/features/caps_word)

```
QK_CAPS_WORD_TOGGLE
```

```
CW_TOGG
```

## Dynamic Macros [​](https://docs.qmk.fm/keycodes#dynamic-macros)

[​](https://docs.qmk.fm/keycodes#dynamic-macros)
See also: [Dynamic Macros](https://docs.qmk.fm/features/dynamic_macros)

```
QK_DYNAMIC_MACRO_RECORD_START_1
```

```
DM_REC1
```

```
QK_DYNAMIC_MACRO_RECORD_START_2
```

```
DM_REC2
```

```
QK_DYNAMIC_MACRO_PLAY_1
```

```
DM_PLY1
```

```
QK_DYNAMIC_MACRO_PLAY_2
```

```
DM_PLY2
```

```
QK_DYNAMIC_MACRO_RECORD_STOP
```

```
DM_RSTP
```

## Grave Escape [​](https://docs.qmk.fm/keycodes#grave-escape)

[​](https://docs.qmk.fm/keycodes#grave-escape)
See also: [Grave Escape](https://docs.qmk.fm/features/grave_esc)

```
QK_GRAVE_ESCAPE
```

```
QK_GESC
```

```
`
```

## Joystick [​](https://docs.qmk.fm/keycodes#joystick)

[​](https://docs.qmk.fm/keycodes#joystick)
See also: [Joystick](https://docs.qmk.fm/features/joystick)

```
QK_JOYSTICK_BUTTON_0
```

```
JS_0
```

```
QK_JOYSTICK_BUTTON_1
```

```
JS_1
```

```
QK_JOYSTICK_BUTTON_2
```

```
JS_2
```

```
QK_JOYSTICK_BUTTON_3
```

```
JS_3
```

```
QK_JOYSTICK_BUTTON_4
```

```
JS_4
```

```
QK_JOYSTICK_BUTTON_5
```

```
JS_5
```

```
QK_JOYSTICK_BUTTON_6
```

```
JS_6
```

```
QK_JOYSTICK_BUTTON_7
```

```
JS_7
```

```
QK_JOYSTICK_BUTTON_8
```

```
JS_8
```

```
QK_JOYSTICK_BUTTON_9
```

```
JS_9
```

```
QK_JOYSTICK_BUTTON_10
```

```
JS_10
```

```
QK_JOYSTICK_BUTTON_11
```

```
JS_11
```

```
QK_JOYSTICK_BUTTON_12
```

```
JS_12
```

```
QK_JOYSTICK_BUTTON_13
```

```
JS_13
```

```
QK_JOYSTICK_BUTTON_14
```

```
JS_14
```

```
QK_JOYSTICK_BUTTON_15
```

```
JS_15
```

```
QK_JOYSTICK_BUTTON_16
```

```
JS_16
```

```
QK_JOYSTICK_BUTTON_17
```

```
JS_17
```

```
QK_JOYSTICK_BUTTON_18
```

```
JS_18
```

```
QK_JOYSTICK_BUTTON_19
```

```
JS_19
```

```
QK_JOYSTICK_BUTTON_20
```

```
JS_20
```

```
QK_JOYSTICK_BUTTON_21
```

```
JS_21
```

```
QK_JOYSTICK_BUTTON_22
```

```
JS_22
```

```
QK_JOYSTICK_BUTTON_23
```

```
JS_23
```

```
QK_JOYSTICK_BUTTON_24
```

```
JS_24
```

```
QK_JOYSTICK_BUTTON_25
```

```
JS_25
```

```
QK_JOYSTICK_BUTTON_26
```

```
JS_26
```

```
QK_JOYSTICK_BUTTON_27
```

```
JS_27
```

```
QK_JOYSTICK_BUTTON_28
```

```
JS_28
```

```
QK_JOYSTICK_BUTTON_29
```

```
JS_29
```

```
QK_JOYSTICK_BUTTON_30
```

```
JS_30
```

```
QK_JOYSTICK_BUTTON_31
```

```
JS_31
```

## Key Lock [​](https://docs.qmk.fm/keycodes#key-lock)

[​](https://docs.qmk.fm/keycodes#key-lock)
See also: [Key Lock](https://docs.qmk.fm/features/key_lock)

```
QK_LOCK
```

## Layer Lock [​](https://docs.qmk.fm/keycodes#layer-lock)

[​](https://docs.qmk.fm/keycodes#layer-lock)
See also: [Layer Lock](https://docs.qmk.fm/features/layer_lock)

```
QK_LAYER_LOCK
```

```
QK_LLCK
```

## Layer Switching [​](https://docs.qmk.fm/keycodes#layer-switching)

[​](https://docs.qmk.fm/keycodes#layer-switching)
See also: [Layer Switching](https://docs.qmk.fm/feature_layers#switching-and-toggling-layers)

```
DF(layer)
```

```
PDF(layer)
```

```
MO(layer)
```

```
layer
```

```
KC_TRNS
```

```
LM(layer, mod)
```

```
layer
```

```
mod
```

```
mod
```

[here](https://docs.qmk.fm/mod_tap)

```
LM(LAYER_1, MOD_LALT)
```

```
LT(layer, kc)
```

```
layer
```

```
kc
```

```
TG(layer)
```

```
layer
```

```
TO(layer)
```

```
layer
```

```
TT(layer)
```

```
layer
```

## Leader Key [​](https://docs.qmk.fm/keycodes#leader-key)

[​](https://docs.qmk.fm/keycodes#leader-key)
See also: [Leader Key](https://docs.qmk.fm/features/leader_key)

```
QK_LEAD
```

## LED Matrix [​](https://docs.qmk.fm/keycodes#led-matrix)

[​](https://docs.qmk.fm/keycodes#led-matrix)
See also: [LED Matrix](https://docs.qmk.fm/features/led_matrix)

```
QK_LED_MATRIX_ON
```

```
LM_ON
```

```
QK_LED_MATRIX_OFF
```

```
LM_OFF
```

```
QK_LED_MATRIX_TOGGLE
```

```
LM_TOGG
```

```
QK_LED_MATRIX_MODE_NEXT
```

```
LM_NEXT
```

```
QK_LED_MATRIX_MODE_PREVIOUS
```

```
LM_PREV
```

```
QK_LED_MATRIX_BRIGHTNESS_UP
```

```
LM_BRIU
```

```
QK_LED_MATRIX_BRIGHTNESS_DOWN
```

```
LM_BRID
```

```
QK_LED_MATRIX_SPEED_UP
```

```
LM_SPDU
```

```
QK_LED_MATRIX_SPEED_DOWN
```

```
LM_SPDD
```

```
QK_LED_MATRIX_FLAG_NEXT
```

```
LM_FLGN
```

```
QK_LED_MATRIX_FLAG_PREVIOUS
```

```
LM_FLGP
```

[​](https://docs.qmk.fm/keycodes#magic-keycodes)
See also: [Magic Keycodes](https://docs.qmk.fm/keycodes_magic)

```
QK_MAGIC_SWAP_CONTROL_CAPS_LOCK
```

```
CL_SWAP
```

```
QK_MAGIC_UNSWAP_CONTROL_CAPS_LOCK
```

```
CL_NORM
```

```
QK_MAGIC_TOGGLE_CONTROL_CAPS_LOCK
```

```
CL_TOGG
```

```
QK_MAGIC_CAPS_LOCK_AS_CONTROL_ON
```

```
CL_CTRL
```

```
QK_MAGIC_CAPS_LOCK_AS_CONTROL_OFF
```

```
CL_CAPS
```

```
QK_MAGIC_SWAP_ESCAPE_CAPS_LOCK
```

```
EC_SWAP
```

```
QK_MAGIC_UNSWAP_ESCAPE_CAPS_LOCK
```

```
EC_NORM
```

```
QK_MAGIC_TOGGLE_ESCAPE_CAPS_LOCK
```

```
EC_TOGG
```

```
QK_MAGIC_SWAP_LCTL_LGUI
```

```
CG_LSWP
```

```
QK_MAGIC_UNSWAP_LCTL_LGUI
```

```
CG_LNRM
```

```
QK_MAGIC_SWAP_RCTL_RGUI
```

```
CG_RSWP
```

```
QK_MAGIC_UNSWAP_RCTL_RGUI
```

```
CG_RNRM
```

```
QK_MAGIC_SWAP_CTL_GUI
```

```
CG_SWAP
```

```
QK_MAGIC_UNSWAP_CTL_GUI
```

```
CG_NORM
```

```
QK_MAGIC_TOGGLE_CTL_GUI
```

```
CG_TOGG
```

```
QK_MAGIC_SWAP_LALT_LGUI
```

```
AG_LSWP
```

```
QK_MAGIC_UNSWAP_LALT_LGUI
```

```
AG_LNRM
```

```
QK_MAGIC_SWAP_RALT_RGUI
```

```
AG_RSWP
```

```
QK_MAGIC_UNSWAP_RALT_RGUI
```

```
AG_RNRM
```

```
QK_MAGIC_SWAP_ALT_GUI
```

```
AG_SWAP
```

```
QK_MAGIC_UNSWAP_ALT_GUI
```

```
AG_NORM
```

```
QK_MAGIC_TOGGLE_ALT_GUI
```

```
AG_TOGG
```

```
QK_MAGIC_GUI_OFF
```

```
GU_OFF
```

```
QK_MAGIC_GUI_ON
```

```
GU_ON
```

```
QK_MAGIC_TOGGLE_GUI
```

```
GU_TOGG
```

```
QK_MAGIC_SWAP_GRAVE_ESC
```

```
GE_SWAP
```

```
`
```

```
QK_MAGIC_UNSWAP_GRAVE_ESC
```

```
GE_NORM
```

```
`
```

```
QK_MAGIC_SWAP_BACKSLASH_BACKSPACE
```

```
BS_SWAP
```

```
\
```

```
QK_MAGIC_UNSWAP_BACKSLASH_BACKSPACE
```

```
BS_NORM
```

```
\
```

```
QK_MAGIC_TOGGLE_BACKSLASH_BACKSPACE
```

```
BS_TOGG
```

```
\
```

```
QK_MAGIC_NKRO_ON
```

```
NK_ON
```

```
QK_MAGIC_NKRO_OFF
```

```
NK_OFF
```

```
QK_MAGIC_TOGGLE_NKRO
```

```
NK_TOGG
```

```
QK_MAGIC_EE_HANDS_LEFT
```

```
EH_LEFT
```

```
EE_HANDS
```

```
QK_MAGIC_EE_HANDS_RIGHT
```

```
EH_RGHT
```

```
EE_HANDS
```

[​](https://docs.qmk.fm/keycodes#midi)
See also: [MIDI](https://docs.qmk.fm/features/midi)

```
QK_MIDI_ON
```

```
MI_ON
```

```
QK_MIDI_OFF
```

```
MI_OFF
```

```
QK_MIDI_TOGGLE
```

```
MI_TOGG
```

```
QK_MIDI_NOTE_C_0
```

```
MI_C
```

```
QK_MIDI_NOTE_C_SHARP_0
```

```
MI_Cs
```

```
MI_Db
```

```
QK_MIDI_NOTE_D_0
```

```
MI_D
```

```
QK_MIDI_NOTE_D_SHARP_0
```

```
MI_Ds
```

```
MI_Eb
```

```
QK_MIDI_NOTE_E_0
```

```
MI_E
```

```
QK_MIDI_NOTE_F_0
```

```
MI_F
```

```
QK_MIDI_NOTE_F_SHARP_0
```

```
MI_Fs
```

```
MI_Gb
```

```
QK_MIDI_NOTE_G_0
```

```
MI_G
```

```
QK_MIDI_NOTE_G_SHARP_0
```

```
MI_Gs
```

```
MI_Ab
```

```
QK_MIDI_NOTE_A_0
```

```
MI_A
```

```
QK_MIDI_NOTE_A_SHARP_0
```

```
MI_As
```

```
MI_Bb
```

```
QK_MIDI_NOTE_B_0
```

```
MI_B
```

```
QK_MIDI_NOTE_C_1
```

```
MI_C1
```

```
QK_MIDI_NOTE_C_SHARP_1
```

```
MI_Cs1
```

```
MI_Db1
```

```
QK_MIDI_NOTE_D_1
```

```
MI_D1
```

```
QK_MIDI_NOTE_D_SHARP_1
```

```
MI_Ds1
```

```
MI_Eb1
```

```
QK_MIDI_NOTE_E_1
```

```
MI_E1
```

```
QK_MIDI_NOTE_F_1
```

```
MI_F1
```

```
QK_MIDI_NOTE_F_SHARP_1
```

```
MI_Fs1
```

```
MI_Gb1
```

```
QK_MIDI_NOTE_G_1
```

```
MI_G1
```

```
QK_MIDI_NOTE_G_SHARP_1
```

```
MI_Gs1
```

```
MI_Ab1
```

```
QK_MIDI_NOTE_A_1
```

```
MI_A1
```

```
QK_MIDI_NOTE_A_SHARP_1
```

```
MI_As1
```

```
MI_Bb1
```

```
QK_MIDI_NOTE_B_1
```

```
MI_B1
```

```
QK_MIDI_NOTE_C_2
```

```
MI_C2
```

```
QK_MIDI_NOTE_C_SHARP_2
```

```
MI_Cs2
```

```
MI_Db2
```

```
QK_MIDI_NOTE_D_2
```

```
MI_D2
```

```
QK_MIDI_NOTE_D_SHARP_2
```

```
MI_Ds2
```

```
MI_Eb2
```

```
QK_MIDI_NOTE_E_2
```

```
MI_E2
```

```
QK_MIDI_NOTE_F_2
```

```
MI_F2
```

```
QK_MIDI_NOTE_F_SHARP_2
```

```
MI_Fs2
```

```
MI_Gb2
```

```
QK_MIDI_NOTE_G_2
```

```
MI_G2
```

```
QK_MIDI_NOTE_G_SHARP_2
```

```
MI_Gs2
```

```
MI_Ab2
```

```
QK_MIDI_NOTE_A_2
```

```
MI_A2
```

```
QK_MIDI_NOTE_A_SHARP_2
```

```
MI_As2
```

```
MI_Bb2
```

```
QK_MIDI_NOTE_B_2
```

```
MI_B2
```

```
QK_MIDI_NOTE_C_3
```

```
MI_C3
```

```
QK_MIDI_NOTE_C_SHARP_3
```

```
MI_Cs3
```

```
MI_Db3
```

```
QK_MIDI_NOTE_D_3
```

```
MI_D3
```

```
QK_MIDI_NOTE_D_SHARP_3
```

```
MI_Ds3
```

```
MI_Eb3
```

```
QK_MIDI_NOTE_E_3
```

```
MI_E3
```

```
QK_MIDI_NOTE_F_3
```

```
MI_F3
```

```
QK_MIDI_NOTE_F_SHARP_3
```

```
MI_Fs3
```

```
MI_Gb3
```

```
QK_MIDI_NOTE_G_3
```

```
MI_G3
```

```
QK_MIDI_NOTE_G_SHARP_3
```

```
MI_Gs3
```

```
MI_Ab3
```

```
QK_MIDI_NOTE_A_3
```

```
MI_A3
```

```
QK_MIDI_NOTE_A_SHARP_3
```

```
MI_As3
```

```
MI_Bb3
```

```
QK_MIDI_NOTE_B_3
```

```
MI_B3
```

```
QK_MIDI_NOTE_C_4
```

```
MI_C4
```

```
QK_MIDI_NOTE_C_SHARP_4
```

```
MI_Cs4
```

```
MI_Db4
```

```
QK_MIDI_NOTE_D_4
```

```
MI_D4
```

```
QK_MIDI_NOTE_D_SHARP_4
```

```
MI_Ds4
```

```
MI_Eb4
```

```
QK_MIDI_NOTE_E_4
```

```
MI_E4
```

```
QK_MIDI_NOTE_F_4
```

```
MI_F4
```

```
QK_MIDI_NOTE_F_SHARP_4
```

```
MI_Fs4
```

```
MI_Gb4
```

```
QK_MIDI_NOTE_G_4
```

```
MI_G4
```

```
QK_MIDI_NOTE_G_SHARP_4
```

```
MI_Gs4
```

```
MI_Ab4
```

```
QK_MIDI_NOTE_A_4
```

```
MI_A4
```

```
QK_MIDI_NOTE_A_SHARP_4
```

```
MI_As4
```

```
MI_Bb4
```

```
QK_MIDI_NOTE_B_4
```

```
MI_B4
```

```
QK_MIDI_NOTE_C_5
```

```
MI_C5
```

```
QK_MIDI_NOTE_C_SHARP_5
```

```
MI_Cs5
```

```
MI_Db5
```

```
QK_MIDI_NOTE_D_5
```

```
MI_D5
```

```
QK_MIDI_NOTE_D_SHARP_5
```

```
MI_Ds5
```

```
MI_Eb5
```

```
QK_MIDI_NOTE_E_5
```

```
MI_E5
```

```
QK_MIDI_NOTE_F_5
```

```
MI_F5
```

```
QK_MIDI_NOTE_F_SHARP_5
```

```
MI_Fs5
```

```
MI_Gb5
```

```
QK_MIDI_NOTE_G_5
```

```
MI_G5
```

```
QK_MIDI_NOTE_G_SHARP_5
```

```
MI_Gs5
```

```
MI_Ab5
```

```
QK_MIDI_NOTE_A_5
```

```
MI_A5
```

```
QK_MIDI_NOTE_A_SHARP_5
```

```
MI_As5
```

```
MI_Bb5
```

```
QK_MIDI_NOTE_B_5
```

```
MI_B5
```

```
QK_MIDI_OCTAVE_N2
```

```
MI_OCN2
```

```
QK_MIDI_OCTAVE_N1
```

```
MI_OCN1
```

```
QK_MIDI_OCTAVE_0
```

```
MI_OC0
```

```
QK_MIDI_OCTAVE_1
```

```
MI_OC1
```

```
QK_MIDI_OCTAVE_2
```

```
MI_OC2
```

```
QK_MIDI_OCTAVE_3
```

```
MI_OC3
```

```
QK_MIDI_OCTAVE_4
```

```
MI_OC4
```

```
QK_MIDI_OCTAVE_5
```

```
MI_OC5
```

```
QK_MIDI_OCTAVE_6
```

```
MI_OC6
```

```
QK_MIDI_OCTAVE_7
```

```
MI_OC7
```

```
QK_MIDI_OCTAVE_DOWN
```

## MIDI [​](https://docs.qmk.fm/keycodes#midi)

```
MI_OCTD
```

```
QK_MIDI_OCTAVE_UP
```

```
MI_OCTU
```

```
QK_MIDI_TRANSPOSE_N6
```

```
MI_TRN6
```

```
QK_MIDI_TRANSPOSE_N5
```

```
MI_TRN5
```

```
QK_MIDI_TRANSPOSE_N4
```

```
MI_TRN4
```

```
QK_MIDI_TRANSPOSE_N3
```

```
MI_TRN3
```

```
QK_MIDI_TRANSPOSE_N2
```

```
MI_TRN2
```

```
QK_MIDI_TRANSPOSE_N1
```

```
MI_TRN1
```

```
QK_MIDI_TRANSPOSE_0
```

```
MI_TR0
```

```
QK_MIDI_TRANSPOSE_1
```

```
MI_TR1
```

```
QK_MIDI_TRANSPOSE_2
```

```
MI_TR2
```

```
QK_MIDI_TRANSPOSE_3
```

```
MI_TR3
```

```
QK_MIDI_TRANSPOSE_4
```

```
MI_TR4
```

```
QK_MIDI_TRANSPOSE_5
```

```
MI_TR5
```

```
QK_MIDI_TRANSPOSE_6
```

```
MI_TR6
```

```
QK_MIDI_TRANSPOSE_DOWN
```

```
MI_TRSD
```

```
QK_MIDI_TRANSPOSE_UP
```

```
MI_TRSU
```

```
QK_MIDI_VELOCITY_0
```

```
MI_VL0
```

```
QK_MIDI_VELOCITY_1
```

```
MI_VL1
```

```
QK_MIDI_VELOCITY_2
```

```
MI_VL2
```

```
QK_MIDI_VELOCITY_3
```

```
MI_VL3
```

```
QK_MIDI_VELOCITY_4
```

```
MI_VL4
```

```
QK_MIDI_VELOCITY_5
```

```
MI_VL5
```

```
QK_MIDI_VELOCITY_6
```

```
MI_VL6
```

```
QK_MIDI_VELOCITY_7
```

```
MI_VL7
```

```
QK_MIDI_VELOCITY_8
```

```
MI_VL8
```

```
QK_MIDI_VELOCITY_9
```

```
MI_VL9
```

```
QK_MIDI_VELOCITY_10
```

```
MI_VL10
```

```
QK_MIDI_VELOCITY_DOWN
```

```
MI_VELD
```

```
QK_MIDI_VELOCITY_UP
```

```
MI_VELU
```

```
QK_MIDI_CHANNEL_1
```

```
MI_CH1
```

```
QK_MIDI_CHANNEL_2
```

```
MI_CH2
```

```
QK_MIDI_CHANNEL_3
```

```
MI_CH3
```

```
QK_MIDI_CHANNEL_4
```

```
MI_CH4
```

```
QK_MIDI_CHANNEL_5
```

```
MI_CH5
```

```
QK_MIDI_CHANNEL_6
```

```
MI_CH6
```

```
QK_MIDI_CHANNEL_7
```

```
MI_CH7
```

```
QK_MIDI_CHANNEL_8
```

```
MI_CH8
```

```
QK_MIDI_CHANNEL_9
```

```
MI_CH9
```

```
QK_MIDI_CHANNEL_10
```

```
MI_CH10
```

```
QK_MIDI_CHANNEL_11
```

```
MI_CH11
```

```
QK_MIDI_CHANNEL_12
```

```
MI_CH12
```

```
QK_MIDI_CHANNEL_13
```

```
MI_CH13
```

```
QK_MIDI_CHANNEL_14
```

```
MI_CH14
```

```
QK_MIDI_CHANNEL_15
```

```
MI_CH15
```

```
QK_MIDI_CHANNEL_16
```

```
MI_CH16
```

```
QK_MIDI_CHANNEL_DOWN
```

```
MI_CHND
```

```
QK_MIDI_CHANNEL_UP
```

```
MI_CHNU
```

```
QK_MIDI_ALL_NOTES_OFF
```

```
MI_AOFF
```

```
QK_MIDI_SUSTAIN
```

```
MI_SUST
```

```
QK_MIDI_PORTAMENTO
```

```
MI_PORT
```

```
QK_MIDI_SOSTENUTO
```

```
MI_SOST
```

```
QK_MIDI_SOFT
```

```
MI_SOFT
```

```
QK_MIDI_LEGATO
```

```
MI_LEG
```

```
QK_MIDI_MODULATION
```

```
MI_MOD
```

```
QK_MIDI_MODULATION_SPEED_DOWN
```

```
MI_MODD
```

```
QK_MIDI_MODULATION_SPEED_UP
```

```
MI_MODU
```

```
QK_MIDI_PITCH_BEND_DOWN
```

```
MI_BNDD
```

```
QK_MIDI_PITCH_BEND_UP
```

```
MI_BNDU
```

## Mouse Keys [​](https://docs.qmk.fm/keycodes#mouse-keys)

[​](https://docs.qmk.fm/keycodes#mouse-keys)
See also: [Mouse Keys](https://docs.qmk.fm/features/mouse_keys)

```
QK_MOUSE_CURSOR_UP
```

```
MS_UP
```

```
QK_MOUSE_CURSOR_DOWN
```

```
MS_DOWN
```

```
QK_MOUSE_CURSOR_LEFT
```

```
MS_LEFT
```

```
QK_MOUSE_CURSOR_RIGHT
```

```
MS_RGHT
```

```
QK_MOUSE_BUTTON_1
```

```
MS_BTN1
```

```
QK_MOUSE_BUTTON_2
```

```
MS_BTN2
```

```
QK_MOUSE_BUTTON_3
```

```
MS_BTN3
```

```
QK_MOUSE_BUTTON_4
```

```
MS_BTN4
```

```
QK_MOUSE_BUTTON_5
```

```
MS_BTN5
```

```
QK_MOUSE_BUTTON_6
```

```
MS_BTN6
```

```
QK_MOUSE_BUTTON_7
```

```
MS_BTN7
```

```
QK_MOUSE_BUTTON_8
```

```
MS_BTN8
```

```
QK_MOUSE_WHEEL_UP
```

```
MS_WHLU
```

```
QK_MOUSE_WHEEL_DOWN
```

```
MS_WHLD
```

```
QK_MOUSE_WHEEL_LEFT
```

```
MS_WHLL
```

```
QK_MOUSE_WHEEL_RIGHT
```

```
MS_WHLR
```

```
QK_MOUSE_ACCELERATION_0
```

```
MS_ACL0
```

```
QK_MOUSE_ACCELERATION_1
```

```
MS_ACL1
```

```
QK_MOUSE_ACCELERATION_2
```

```
MS_ACL2
```

## Modifiers [​](https://docs.qmk.fm/keycodes#modifiers)

[​](https://docs.qmk.fm/keycodes#modifiers)
See also: [Modifier Keys](https://docs.qmk.fm/feature_advanced_keycodes#modifier-keys)

```
LCTL(kc)
```

```
C(kc)
```

```
kc
```

```
LSFT(kc)
```

```
S(kc)
```

```
kc
```

```
LALT(kc)
```

```
A(kc)
```

```
LOPT(kc)
```

```
kc
```

```
LGUI(kc)
```

```
G(kc)
```

```
LCMD(kc)
```

```
LWIN(kc)
```

```
kc
```

```
LCS(kc)
```

```
kc
```

```
LCA(kc)
```

```
kc
```

```
LCG(kc)
```

```
kc
```

```
LSA(kc)
```

```
kc
```

```
LSG(kc)
```

```
kc
```

```
LAG(kc)
```

```
kc
```

```
LCSG(kc)
```

```
kc
```

```
LCAG(kc)
```

```
kc
```

```
LSAG(kc)
```

```
kc
```

```
RCTL(kc)
```

```
kc
```

```
RSFT(kc)
```

```
kc
```

```
RALT(kc)
```

```
ROPT(kc)
```

```
ALGR(kc)
```

```
kc
```

```
RGUI(kc)
```

```
RCMD(kc)
```

```
RWIN(kc)
```

```
kc
```

```
RCS(kc)
```

```
kc
```

```
RCA(kc)
```

```
kc
```

```
RCG(kc)
```

```
kc
```

```
RSA(kc)
```

```
kc
```

```
RSG(kc)
```

```
kc
```

```
RAG(kc)
```

```
kc
```

```
RCSG(kc)
```

```
kc
```

```
RCAG(kc)
```

```
kc
```

```
RSAG(kc)
```

```
kc
```

```
MEH(kc)
```

```
kc
```

```
HYPR(kc)
```

```
kc
```

```
KC_MEH
```

```
KC_HYPR
```

## Mod-Tap Keys [​](https://docs.qmk.fm/keycodes#mod-tap-keys)

[​](https://docs.qmk.fm/keycodes#mod-tap-keys)
See also: [Mod-Tap](https://docs.qmk.fm/mod_tap)

```
MT(mod, kc)
```

```
mod
```

```
kc
```

```
LCTL_T(kc)
```

```
CTL_T(kc)
```

```
kc
```

```
LSFT_T(kc)
```

```
SFT_T(kc)
```

```
kc
```

```
LALT_T(kc)
```

```
ALT_T(kc)
```

```
LOPT_T(kc)
```

```
OPT_T(kc)
```

```
kc
```

```
LGUI_T(kc)
```

```
GUI_T(kc)
```

```
LCMD_T(kc)
```

```
LWIN_T(kc)
```

```
CMD_T(kc)
```

```
WIN_T(kc)
```

```
kc
```

```
LCS_T(kc)
```

```
kc
```

```
LCA_T(kc)
```

```
kc
```

```
LCG_T(kc)
```

```
kc
```

```
LSA_T(kc)
```

```
kc
```

```
LSG_T(kc)
```

```
kc
```

```
LAG_T(kc)
```

```
kc
```

```
LCSG_T(kc)
```

```
kc
```

```
LCAG_T(kc)
```

```
kc
```

```
LSAG_T(kc)
```

```
kc
```

```
RCTL_T(kc)
```

```
kc
```

```
RSFT_T(kc)
```

```
kc
```

```
RALT_T(kc)
```

```
ROPT_T(kc)
```

```
ALGR_T(kc)
```

```
kc
```

```
RGUI_T(kc)
```

```
RCMD_T(kc)
```

```
RWIN_T(kc)
```

```
kc
```

```
RCS_T(kc)
```

```
kc
```

```
RCA_T(kc)
```

```
kc
```

```
RCG_T(kc)
```

```
kc
```

```
RSA_T(kc)
```

```
kc
```

```
RSG_T(kc)
```

```
kc
```

```
RAG_T(kc)
```

```
kc
```

```
RCSG_T(kc)
```

```
kc
```

```
RCAG_T(kc)
```

```
kc
```

```
RSAG_T(kc)
```

```
kc
```

```
MEH_T(kc)
```

```
kc
```

```
HYPR_T(kc)
```

```
kc
```

## Tapping Term Keys [​](https://docs.qmk.fm/keycodes#tapping-term-keys)

[​](https://docs.qmk.fm/keycodes#tapping-term-keys)
See also: [Dynamic Tapping Term](https://docs.qmk.fm/tap_hold#dynamic-tapping-term)

```
QK_DYNAMIC_TAPPING_TERM_PRINT
```

```
DT_PRNT
```

```
QK_DYNAMIC_TAPPING_TERM_UP
```

```
DT_UP
```

```
DYNAMIC_TAPPING_TERM_INCREMENT
```

```
QK_DYNAMIC_TAPPING_TERM_DOWN
```

```
DT_DOWN
```

```
DYNAMIC_TAPPING_TERM_INCREMENT
```

## RGB Lighting [​](https://docs.qmk.fm/keycodes#rgb-lighting)

[​](https://docs.qmk.fm/keycodes#rgb-lighting)
See also: [RGB Lighting](https://docs.qmk.fm/features/rgblight)

```
QK_UNDERGLOW_TOGGLE
```

```
UG_TOGG
```

```
QK_UNDERGLOW_MODE_NEXT
```

```
UG_NEXT
```

```
QK_UNDERGLOW_MODE_PREVIOUS
```

```
UG_PREV
```

```
QK_UNDERGLOW_HUE_UP
```

```
UG_HUEU
```

```
QK_UNDERGLOW_HUE_DOWN
```

```
UG_HUED
```

```
QK_UNDERGLOW_SATURATION_UP
```

```
UG_SATU
```

```
QK_UNDERGLOW_SATURATION_DOWN
```

```
UG_SATD
```

```
QK_UNDERGLOW_VALUE_UP
```

```
UG_VALU
```

```
QK_UNDERGLOW_VALUE_DOWN
```

```
UG_VALD
```

```
QK_UNDERGLOW_SPEED_UP
```

```
UG_SPDU
```

```
QK_UNDERGLOW_SPEED_DOWN
```

```
UG_SPDD
```

```
RGB_MODE_PLAIN
```

```
RGB_M_P
```

```
RGB_MODE_BREATHE
```

```
RGB_M_B
```

```
RGB_MODE_RAINBOW
```

```
RGB_M_R
```

```
RGB_MODE_SWIRL
```

```
RGB_M_SW
```

```
RGB_MODE_SNAKE
```

```
RGB_M_SN
```

```
RGB_MODE_KNIGHT
```

```
RGB_M_K
```

```
RGB_MODE_XMAS
```

```
RGB_M_X
```

```
RGB_MODE_GRADIENT
```

```
RGB_M_G
```

```
RGB_MODE_RGBTEST
```

```
RGB_M_T
```

```
RGB_MODE_TWINKLE
```

```
RGB_M_TW
```

## RGB Matrix [​](https://docs.qmk.fm/keycodes#rgb-matrix)

[​](https://docs.qmk.fm/keycodes#rgb-matrix)
See also: [RGB Matrix](https://docs.qmk.fm/features/rgb_matrix)

```
QK_RGB_MATRIX_ON
```

```
RM_ON
```

```
QK_RGB_MATRIX_OFF
```

```
RM_OFF
```

```
QK_RGB_MATRIX_TOGGLE
```

```
RM_TOGG
```

```
QK_RGB_MATRIX_MODE_NEXT
```

```
RM_NEXT
```

```
QK_RGB_MATRIX_MODE_PREVIOUS
```

```
RM_PREV
```

```
QK_RGB_MATRIX_HUE_UP
```

```
RM_HUEU
```

```
QK_RGB_MATRIX_HUE_DOWN
```

```
RM_HUED
```

```
QK_RGB_MATRIX_SATURATION_UP
```

```
RM_SATU
```

```
QK_RGB_MATRIX_SATURATION_DOWN
```

```
RM_SATD
```

```
QK_RGB_MATRIX_VALUE_UP
```

```
RM_VALU
```

```
QK_RGB_MATRIX_VALUE_DOWN
```

```
RM_VALD
```

```
QK_RGB_MATRIX_SPEED_UP
```

```
RM_SPDU
```

```
QK_RGB_MATRIX_SPEED_DOWN
```

```
RM_SPDD
```

```
QK_RGB_MATRIX_FLAG_NEXT
```

```
RM_FLGN
```

```
QK_RGB_MATRIX_FLAG_PREVIOUS
```

```
RM_FLGP
```

## US ANSI Shifted Symbols [​](https://docs.qmk.fm/keycodes#us-ansi-shifted-symbols)

[​](https://docs.qmk.fm/keycodes#us-ansi-shifted-symbols)
See also: [US ANSI Shifted Symbols](https://docs.qmk.fm/keycodes_us_ansi_shifted)

```
KC_TILDE
```

```
KC_TILD
```

```
~
```

```
KC_EXCLAIM
```

```
KC_EXLM
```

```
!
```

```
KC_AT
```

```
@
```

```
KC_HASH
```

```
#
```

```
KC_DOLLAR
```

```
KC_DLR
```

```
$
```

```
KC_PERCENT
```

```
KC_PERC
```

```
%
```

```
KC_CIRCUMFLEX
```

```
KC_CIRC
```

```
^
```

```
KC_AMPERSAND
```

```
KC_AMPR
```

```
&
```

```
KC_ASTERISK
```

```
KC_ASTR
```

```
*
```

```
KC_LEFT_PAREN
```

```
KC_LPRN
```

```
(
```

```
KC_RIGHT_PAREN
```

```
KC_RPRN
```

```
)
```

```
KC_UNDERSCORE
```

```
KC_UNDS
```

```
_
```

```
KC_PLUS
```

```
+
```

```
KC_LEFT_CURLY_BRACE
```

```
KC_LCBR
```

```
{
```

```
KC_RIGHT_CURLY_BRACE
```

```
KC_RCBR
```

```
}
```

```
KC_PIPE
```

```
|
```

```
KC_COLON
```

```
KC_COLN
```

```
:
```

```
KC_DOUBLE_QUOTE
```

```
KC_DQUO
```

```
KC_DQT
```

```
"
```

```
KC_LEFT_ANGLE_BRACKET
```

```
KC_LABK
```

```
KC_LT
```

```
<
```

```
KC_RIGHT_ANGLE_BRACKET
```

```
KC_RABK
```

```
KC_GT
```

```
>
```

```
KC_QUESTION
```

```
KC_QUES
```

```
?
```

## One Shot Keys [​](https://docs.qmk.fm/keycodes#one-shot-keys)

[​](https://docs.qmk.fm/keycodes#one-shot-keys)
See also: [One Shot Keys](https://docs.qmk.fm/one_shot_keys)

```
QK_ONE_SHOT_TOGGLE
```

```
OS_TOGG
```

```
QK_ONE_SHOT_ON
```

```
OS_ON
```

```
QK_ONE_SHOT_OFF
```

```
OS_OFF
```

```
OSL(layer)
```

```
layer
```

```
OSM(mod)
```

```
mod
```

```
OS_LCTL
```

```
OS_LSFT
```

```
OS_LALT
```

```
OS_LGUI
```

```
OS_LCS
```

```
OS_LCA
```

```
OS_LCG
```

```
OS_LSA
```

```
OS_LSG
```

```
OS_LAG
```

```
OS_LCSG
```

```
OS_LCAG
```

```
OS_LSAG
```

```
OS_RCTL
```

```
OS_RSFT
```

```
OS_RALT
```

```
OS_RGUI
```

```
OS_RCS
```

```
OS_RCA
```

```
OS_RCG
```

```
OS_RSA
```

```
OS_RSG
```

```
OS_RAG
```

```
OS_RCSG
```

```
OS_RCAG
```

```
OS_RSAG
```

```
OS_MEH
```

```
OS_HYPR
```

## Programmable Button Support [​](https://docs.qmk.fm/keycodes#programmable-button)

[​](https://docs.qmk.fm/keycodes#programmable-button)
See also: [Programmable Button](https://docs.qmk.fm/features/programmable_button)

```
QK_PROGRAMMABLE_BUTTON_1
```

```
PB_1
```

```
QK_PROGRAMMABLE_BUTTON_2
```

```
PB_2
```

```
QK_PROGRAMMABLE_BUTTON_3
```

```
PB_3
```

```
QK_PROGRAMMABLE_BUTTON_4
```

```
PB_4
```

```
QK_PROGRAMMABLE_BUTTON_5
```

```
PB_5
```

```
QK_PROGRAMMABLE_BUTTON_6
```

```
PB_6
```

```
QK_PROGRAMMABLE_BUTTON_7
```

```
PB_7
```

```
QK_PROGRAMMABLE_BUTTON_8
```

```
PB_8
```

```
QK_PROGRAMMABLE_BUTTON_9
```

```
PB_9
```

```
QK_PROGRAMMABLE_BUTTON_10
```

```
PB_10
```

```
QK_PROGRAMMABLE_BUTTON_11
```

```
PB_11
```

```
QK_PROGRAMMABLE_BUTTON_12
```

```
PB_12
```

```
QK_PROGRAMMABLE_BUTTON_13
```

```
PB_13
```

```
QK_PROGRAMMABLE_BUTTON_14
```

```
PB_14
```

```
QK_PROGRAMMABLE_BUTTON_15
```

```
PB_15
```

```
QK_PROGRAMMABLE_BUTTON_16
```

```
PB_16
```

```
QK_PROGRAMMABLE_BUTTON_17
```

```
PB_17
```

```
QK_PROGRAMMABLE_BUTTON_18
```

```
PB_18
```

```
QK_PROGRAMMABLE_BUTTON_19
```

```
PB_19
```

```
QK_PROGRAMMABLE_BUTTON_20
```

```
PB_20
```

```
QK_PROGRAMMABLE_BUTTON_21
```

```
PB_21
```

```
QK_PROGRAMMABLE_BUTTON_22
```

```
PB_22
```

```
QK_PROGRAMMABLE_BUTTON_23
```

```
PB_23
```

```
QK_PROGRAMMABLE_BUTTON_24
```

```
PB_24
```

```
QK_PROGRAMMABLE_BUTTON_25
```

```
PB_25
```

```
QK_PROGRAMMABLE_BUTTON_26
```

```
PB_26
```

```
QK_PROGRAMMABLE_BUTTON_27
```

```
PB_27
```

```
QK_PROGRAMMABLE_BUTTON_28
```

```
PB_28
```

```
QK_PROGRAMMABLE_BUTTON_29
```

```
PB_29
```

```
QK_PROGRAMMABLE_BUTTON_30
```

```
PB_30
```

```
QK_PROGRAMMABLE_BUTTON_31
```

```
PB_31
```

```
QK_PROGRAMMABLE_BUTTON_32
```

```
PB_32
```

## Repeat Key [​](https://docs.qmk.fm/keycodes#repeat-key)

[​](https://docs.qmk.fm/keycodes#repeat-key)
See also: [Repeat Key](https://docs.qmk.fm/features/repeat_key)

```
QK_REPEAT_KEY
```

```
QK_REP
```

```
QK_ALT_REPEAT_KEY
```

```
QK_AREP
```

## Space Cadet [​](https://docs.qmk.fm/keycodes#space-cadet)

[​](https://docs.qmk.fm/keycodes#space-cadet)
See also: [Space Cadet](https://docs.qmk.fm/features/space_cadet)

```
QK_SPACE_CADET_LEFT_CTRL_PARENTHESIS_OPEN
```

```
SC_LCPO
```

```
(
```

```
QK_SPACE_CADET_RIGHT_CTRL_PARENTHESIS_CLOSE
```

```
SC_RCPC
```

```
)
```

```
QK_SPACE_CADET_LEFT_SHIFT_PARENTHESIS_OPEN
```

```
SC_LSPO
```

```
(
```

```
QK_SPACE_CADET_RIGHT_SHIFT_PARENTHESIS_CLOSE
```

```
SC_RSPC
```

```
)
```

```
QK_SPACE_CADET_LEFT_ALT_PARENTHESIS_OPEN
```

```
SC_LAPO
```

```
(
```

```
QK_SPACE_CADET_RIGHT_ALT_PARENTHESIS_CLOSE
```

```
SC_RAPC
```

```
)
```

```
QK_SPACE_CADET_RIGHT_SHIFT_ENTER
```

```
SC_SENT
```

## Swap Hands [​](https://docs.qmk.fm/keycodes#swap-hands)

[​](https://docs.qmk.fm/keycodes#swap-hands)
See also: [Swap Hands](https://docs.qmk.fm/features/swap_hands)

```
SH_T(kc)
```

```
kc
```

```
QK_SWAP_HANDS_ON
```

```
SH_ON
```

```
QK_SWAP_HANDS_OFF
```

```
SH_OFF
```

```
QK_SWAP_HANDS_MOMENTARY_ON
```

```
SH_MON
```

```
QK_SWAP_HANDS_MOMENTARY_OFF
```

```
SH_MOFF
```

```
QK_SWAP_HANDS_TOGGLE
```

```
SH_TOGG
```

```
QK_SWAP_HANDS_TAP_TOGGLE
```

```
SH_TT
```

```
QK_SWAP_HANDS_ONE_SHOT
```

```
SH_OS
```

## Unicode Support [​](https://docs.qmk.fm/keycodes#unicode-support)

[​](https://docs.qmk.fm/keycodes#unicode-support)
See also: [Unicode Support](https://docs.qmk.fm/features/unicode)

```
UC(c)
```

```
c
```

```
0x7FFF
```

```
UM(i)
```

```
i
```

```
unicode_map
```

```
UP(i, j)
```

```
i
```

```
j
```

```
QK_UNICODE_MODE_NEXT
```

```
UC_NEXT
```

```
QK_UNICODE_MODE_PREVIOUS
```

```
UC_PREV
```

```
QK_UNICODE_MODE_MACOS
```

```
UC_MAC
```

```
QK_UNICODE_MODE_LINUX
```

```
UC_LINX
```

```
QK_UNICODE_MODE_WINDOWS
```

```
UC_WIN
```

```
QK_UNICODE_MODE_BSD
```

```
UC_BSD
```

```
QK_UNICODE_MODE_WINCOMPOSE
```

```
UC_WINC
```

```
QK_UNICODE_MODE_EMACS
```

```
UC_EMAC
```

```
C-x-8 RET
```

[Edit this page](https://github.com/qmk/qmk_firmware/edit/master/docs/keycodes.md)
[Previous pageFixing Your Branch](https://docs.qmk.fm/newbs_git_resynchronize_a_branch)
[Next pageBasic Keycodes](https://docs.qmk.fm/keycodes_basic)
